"""Flask application: routes, MJPEG feed, uploads, alerts and dashboard APIs.

This module is imported by the root app.py launcher.  It wires together the
detector, rule store, zone store, escalation pipeline and frontend templates.
"""
...
from backend import config, contacts_store, escalation, incident_log, notifiers, rules_store, runtime_state, scene_analyst, zones_store
from backend.detector import DetectorThread

app = Flask(__name__, template_folder=str(config.BASE_DIR / "frontend" / "templates"), static_folder=str(config.BASE_DIR / "frontend" / "static"))
Path(config.ALERTS_DIR).mkdir(exist_ok=True)
Path(config.UPLOADS_DIR).mkdir(exist_ok=True)
Path(config.BASE_DIR / "logs").mkdir(exist_ok=True)
...
def current_frame():
    """Return a single JPEG frame from the active video source."""
    ok, frame = _read_frame()
    if not ok:
...
def demo_mode_route():
    """Read or update live camera vs demo/upload mode."""
...
def upload_video():
    """Save an uploaded video to the uploads directory and select it for demo mode."""
...
def start_monitoring():
    """Start (or reconfigure) the background detection thread."""
...
def alerts_json():
    """Return the latest alerts and current monitoring state for the dashboard."""
    with _detector_lock:
...
def alert_image(filename):
    """Serve a saved alert snapshot JPEG."""
    return send_from_directory(config.ALERTS_DIR, filename)


@app.route("/zones")
def get_zones():
    """Return all drawn monitoring zones."""
    return jsonify(zones=zones_store.get_zones())


@app.route("/zones", methods=["POST"])
def create_zone():
    """Create or overwrite a monitoring zone polygon."""
    data = request.get_json(silent=True) or {}
...
def remove_zone(name):
    """Delete a zone and clear its references from any rules."""
...
@app.route("/rules")
def get_rules():
    """Return all configured monitoring rules."""
    return jsonify(rules=rules_store.get_rules())


@app.route("/rules", methods=["POST"])
def create_rule():
    """Create a new standard, PPE or proximity rule."""
    data = request.get_json(silent=True) or {}
...
def set_rule_zone(rule_id):
    """Assign a zone to a rule (or remove the assignment)."""
    data = request.get_json(silent=True) or {}
    zone_name = (data.get("zone_name") or "").strip() or None
    if zone_name and zone_name not in zones_store.get_zones():
        return jsonify(error="Unknown zone"), 400
...
_REFINE_LOG = config.BASE_DIR / "logs" / "refinements.jsonl"
...
@app.route("/contacts")
def get_contacts():
    """Return all configured Telegram escalation contacts."""
    return jsonify(contacts=contacts_store.get_contacts())


@app.route("/contacts", methods=["POST"])
def create_contact():
    """Add a new Telegram contact, optionally bound to a specific zone."""
    data = request.get_json(silent=True) or {}
